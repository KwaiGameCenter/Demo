#pragma once

#if defined(__cplusplus)
extern "C"
{
#endif

#include <inttypes.h>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>

#define RotHi(x, r)		(((x) << (r)) | ((x) >> (32 - (r))))
    
#define U1(x)	( (x) >> 24 )
#define U2(x)	(((x) >> 16 ) & 0xff )
#define U3(x)	(((x) >> 8  ) & 0xff )
#define U4(x)	( (x) & 0xff )
    
#define HU1(x,H)	(((uint32_t) (H)[ U1((x)) ]) << 24)
#define HU2(x,H)	(((uint32_t) (H)[ U2((x)) ]) << 16)
#define HU3(x,H)	(((uint32_t) (H)[ U3((x)) ]) <<  8)
#define HU4(x,H)	(((uint32_t) (H)[ U4((x)) ]))
    
#define G(x,H,r)	RotHi(HU4((x),(H)) | HU3((x),(H)) | HU2((x),(H)) | HU1((x),(H)),(r))
#define SWAP(x,y,tmp)	(tmp) = (x); (x) = (y); (y) = (tmp);
    
    const static uint8_t sSimpleKeyBytes[] =
    {
        0xE6, 0xA2, 0xF3, 0xFF,
        0x1C, 0x1F, 0x6E, 0xA6,
        0x05, 0x29, 0x77, 0x21,
        0x34, 0x82, 0xD5, 0xC0,
        0x06, 0x07, 0x53, 0x16,
        0xED, 0x24, 0x7A, 0x37,
        0x39, 0xCB, 0xA3, 0x83,
        0x03, 0xA9, 0x8B, 0xF6
    };

//    const static int sSimpleKeyLength = sizeof(sSimpleKeyBytes) / sizeof(typeof(uint8_t));

    const static uint8_t H[256] =
    {
        
    };
    
    const static uint32_t KeyIndex[8][7] =
    {
        { 0, 1, 2, 3, 4, 5, 6 },
        { 7, 0, 1, 2, 3, 4, 5 },
        { 6, 7, 0, 1, 2, 3, 4 },
        { 5, 6, 7, 0, 1, 2, 3 },
        { 4, 5, 6, 7, 0, 1, 2 },
        { 3, 4, 5, 6, 7, 0, 1 },
        { 2, 3, 4, 5, 6, 7, 0 },
        { 1, 2, 3, 4, 5, 6, 7 }
    };
    
    __inline__ __attribute__((always_inline))
    void belt_init(const uint8_t* k, int kLen, uint8_t* ks)
    {
        unsigned int i;
        switch(kLen)
        {
                // 128 bits
            case 16:
                for(i = 0; i<4; ++i)
                {
                    ((uint32_t*)ks)[i] = ((uint32_t*)k)[i];
                    ((uint32_t*)ks)[i + 4] = ((uint32_t*)k)[i];
                }
                
                break;
                //192 bits
            case 24:
                for(i = 0; i<6; ++i)
                {
                    ((uint32_t*)ks)[i] = ((uint32_t*)k)[i];
                }
                ((uint32_t*)ks)[6] = (((uint32_t*)k)[0]) ^ (((uint32_t*)k)[1]) ^ (((uint32_t*)k)[2]);
                ((uint32_t*)ks)[7] = (((uint32_t*)k)[3]) ^ (((uint32_t*)k)[4]) ^ (((uint32_t*)k)[5]);
                break;
                //256 bits
            case 32:
                for(i = 0; i<32; ++i) ks[i] = k[i];
                break;
        }
    }
    
    static __inline__ __attribute__((always_inline))
    void belt_encrypt(const uint8_t* ks, const uint8_t* inBlock, uint8_t* outBlock)
    {
        uint32_t a = ((uint32_t *)inBlock)[0];
        uint32_t b = ((uint32_t *)inBlock)[1];
        uint32_t c = ((uint32_t *)inBlock)[2];
        uint32_t d = ((uint32_t *)inBlock)[3];
        uint32_t e;
        int i;
        uint32_t tmp;
        uint32_t * key = (uint32_t*)ks;
        
        for(i = 0; i<8; ++i)
        {
            b ^= G((a + key[KeyIndex[i][0]]), H, 5);
            c ^= G((d + key[KeyIndex[i][1]]), H, 21);
            a = (uint32_t)(a - G((b + key[KeyIndex[i][2]]), H, 13));
            e = (G((b + c + key[KeyIndex[i][3]]), H, 21) ^ (uint32_t)(i + 1));
            b += e;
            c = (uint32_t)(c - e);
            d += G((c + key[KeyIndex[i][4]]), H, 13);
            b ^= G((a + key[KeyIndex[i][5]]), H, 21);
            c ^= G((d + key[KeyIndex[i][6]]), H, 5);
            SWAP(a, b, tmp);
            SWAP(c, d, tmp);
            SWAP(b, c, tmp);
        }
        
        ((uint32_t *)outBlock)[0] = b;
        ((uint32_t *)outBlock)[1] = d;
        ((uint32_t *)outBlock)[2] = a;
        ((uint32_t *)outBlock)[3] = c;
    }
    
    static __inline__ __attribute__((always_inline))
    void belt_decrypt(const uint8_t* ks, const uint8_t* inBlock, uint8_t* outBlock)
    {
        uint32_t a = ((uint32_t *)inBlock)[0];
        uint32_t b = ((uint32_t *)inBlock)[1];
        uint32_t c = ((uint32_t *)inBlock)[2];
        uint32_t d = ((uint32_t *)inBlock)[3];
        uint32_t e;
        int i;
        uint32_t tmp;
        uint32_t * key = (uint32_t*)ks;
        
        for(i = 7; i >= 0; --i)
        {
            b ^= G((a + key[KeyIndex[i][6]]), H, 5);
            c ^= G((d + key[KeyIndex[i][5]]), H, 21);
            a = (uint32_t)(a - G((b + key[KeyIndex[i][4]]), H, 13));
            e = (G((b + c + key[KeyIndex[i][3]]), H, 21) ^ (uint32_t)(i + 1));
            b += e;
            c = (uint32_t)(c - e);
            d += G((c + key[KeyIndex[i][2]]), H, 13);
            b ^= G((a + key[KeyIndex[i][1]]), H, 21);
            c ^= G((d + key[KeyIndex[i][0]]), H, 5);
            SWAP(a, b, tmp);
            SWAP(c, d, tmp);
            SWAP(a, d, tmp);
        }
        
        ((uint32_t *)outBlock)[0] = c;
        ((uint32_t *)outBlock)[1] = a;
        ((uint32_t *)outBlock)[2] = d;
        ((uint32_t *)outBlock)[3] = b;
    }
    
//    static __inline__ __attribute__((always_inline))
//    void SimpleEncrypt(const char* plainText, uint8_t** encryptedBytes, size_t* encryptedSize)
//    {
//        const static size_t sBlockSize = 16;
//
//        uint8_t ks[sSimpleKeyLength] = { 0 };
//        belt_init(sSimpleKeyBytes, sSimpleKeyLength, ks);
//        
//        size_t textLength = strlen(plainText) + 1;
//        size_t blockNum = textLength / sBlockSize;
//        blockNum += ((textLength % sBlockSize) != 0) ? 1 : 0;
//
//        size_t bufferSize = blockNum * sBlockSize;
//        uint8_t* inBuffer = (uint8_t*)calloc(bufferSize, 1);
//        uint8_t* outBuffer = (uint8_t*)malloc(bufferSize);
//        memcpy(inBuffer, plainText, textLength);
//
//        for (size_t i = 0; i < blockNum; ++i)
//        {
//            belt_encrypt(ks, inBuffer + i * sBlockSize, outBuffer + i * sBlockSize);
//        }
//        
//        free(inBuffer);
//        
//        *encryptedBytes = outBuffer;
//        *encryptedSize = bufferSize;
//    }
    
    static __inline__ __attribute__((always_inline))
    char* SimpleDecrypt(const uint8_t* encryptedBytes, size_t encryptedSize)
    {
        const static size_t sBlockSize = 16;
        
        int sSimpleKeyLength = 32;
        uint8_t ks[32] = { 0 };
        belt_init(sSimpleKeyBytes, sSimpleKeyLength, ks);

        if (encryptedSize % sBlockSize != 0)
        {
            return 0;
        }

        size_t blockNum = encryptedSize / sBlockSize;
        const uint8_t* inBuffer = encryptedBytes;
        uint8_t* outBuffer = (uint8_t*)malloc(encryptedSize);

        for (size_t i = 0; i < blockNum; ++i)
        {
            belt_decrypt(ks, inBuffer + i * sBlockSize, outBuffer + i * sBlockSize);
        }

        return (char*)outBuffer;
    }

#if defined(__cplusplus)
}
#endif
